...
temps = np.append(np.array([0.01]), np.arange(10, 1501, 10))
ds = np.array([d(temp, s, d_max) for temp in temps])

status = Queue()
progress = collections.OrderedDict()

proteins = [Protein(15, temps[i], ds[i], status) for i in range(0, len(temps))]
...
protein = Protein(15, 1500, 600, status)
temps = np.arange(1470, 30, -30)
...
space = np.arange(0, 600*(len(temps) + 1) + 1)

for i in space:
    averages.append(np.average(protein.energies[:i + 1]))



plt.plot(space, protein.energies)
