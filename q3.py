...
proteins = [Protein(10, temps[i], ds[i], status) for i in range(0, len(temps))]
...
for protein in proteins:
    protein.join()
    averages.append(np.average(np.sqrt(protein.diameters)))
...
plt.ylabel(r"$\langle L \rangle$", size=14)
plt.xlabel(r"$T$ / K", size=14)
plt.title("Mean largest diameter versus temperature (took {0}s)".format(time_taken))
